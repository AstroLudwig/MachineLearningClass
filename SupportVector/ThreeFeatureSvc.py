...
blue_x = np.arange(.1, 3, .1) 
blue_y = np.arange(10, 15, (15-10)/len(blue_x)) 
blue_z = np.arange(4,9,(9-4)/len(blue_x))
...
black_x = np.arange(2.7, 6, .1) 
black_y = np.arange(5, 12, (12-5)/len(black_x)) 
black_z = np.arange(4.15,9,(9-4.15)/len(black_x))

random.shuffle(black_y); random.shuffle(black_x)
random.shuffle(black_z)
 

data = np.zeros([len(blue_y)+len(black_y),3])
data[:,0] = np.append(blue_x,black_x)
data[:,1] = np.append(blue_y,black_y)
data[:,2] = np.append(blue_z,black_z)

labels = np.append(np.repeat("Blue",len(blue_y)),np.repeat("Black",len(black_y)))

param_x,param_y,param_z = np.meshgrid(np.arange(0,7,.1),np.arange(3,16,.1),np.arange(3,10,.1),indexing="ij")

param_data = np.c_[param_x.ravel(),param_y.ravel(),param_z.ravel()]
...
decision_func = svc.decision_function(param_data)
